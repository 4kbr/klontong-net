...
`docs/ARCHITECTURE.md` tabel modul, `docs/DECISIONS.md` ADR yang menjelaskan
kenapa ini modul terpisah.
...
```bash
make -C apps/backend migrate-version
make -C apps/backend migrate-force version=5
make migrate
```
